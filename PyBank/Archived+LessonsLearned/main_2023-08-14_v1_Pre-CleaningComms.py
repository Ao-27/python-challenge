...
#path1_data = open(file_path1, 'r') as file:        #with open(file_path1, 'r') as file:
#KEEPING ABOVE as "lesson learned" ... the "why" to be fixed/explored more in the future.

#with open(file_path1, 'r') as csv_file1:
#     csv_reader_file1 = csv.reader(csv_file1, delimiter=',')
#     csv1_header = next(csv_reader_file1)
#     print(csv1_header)  # Skipping header row based on .csv manual review.
     #print(csv_reader_file1)
...
with open(file_path1, 'r',encoding='utf-8-sig',newline='') as csv_file1a: #with open("myfile.csv") as f:
    #csv1_headerA = [header.strip() for header in next(csv_file1a).split(",")[1:]]  #next(f).split(",")[1:]] #REPALCED with my own syntax for personal readability.
     readerA = csv.DictReader(file_path1)
