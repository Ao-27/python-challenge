...
with open(file_path2, 'r',encoding='utf-8-sig',newline='') as csv_file2:
     ...
     for count,row_1a in enumerate(csv_reader,1):  #via URL => https://stackoverflow.com/questions/28973207/get-length-of-csv-file-without-ruining-reader
          ...
          #Preparation for Change Calculation by storing the prior profit_loss_col ... to be updated in the future by the "inner" loop being place above this code line...
          prior_rows_prof_loss = prior_rows_prof_loss + profit_loss_col
          #Not writing to the DICT to add clarity that the final value is written within the "inner loop".
#COMPLETE via URL => https://github.com/ofunkey/python-challenge/blob/master/PyBank/main.py ... jot down what way to accomplish this via the DICT approach ... 
...
for prev,curr in zip(Prev_Curr_Nxt_RollTotal_v2, Prev_Curr_Nxt_RollTotal_v2[1:]):
     ...
     if int(prev[0]) > 0 and int(prev[1]) > 0 and  int(curr[1]) > 0:
          ...
          Loop_PlaceHolder.append(changed_difference)
               #print(Records_forAnalysis)
print(Loop_PlaceHolder)  #BROKEN ... need to transition to ... schedule issues + setting up football coaching steps ... 
#print("+-+-+-+")
