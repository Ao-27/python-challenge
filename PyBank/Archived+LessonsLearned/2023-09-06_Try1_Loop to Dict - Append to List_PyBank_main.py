...
Calculation_Tuple = [(d["Row_ID"],d[header1a[1]]) for d in Loop_PlaceHolder]  #,v in Loop_PlaceHolder] # if v.get(header1a[1].value() > 0)]
#ERROR ABOVE ... need to somehow target the "key:value pair..."

print(Calculation_Tuple) #WORKs ... 
print("+-+-+-+")

Calc_Range = len(Calculation_Tuple)
print(Calc_Range)

for item in range(1,Calc_Range): #CREATE "box" to operate in ... REMINDER "Row_ID" is representing each month's change ... as a validator.
     Calculations_For_Analysis = {}
     Prior_Prof_Loss = 0
     Calc_change_monthly = 0
     for calc_x in Calculation_Tuple: #CREATE "calculation" for each row within the "box" ...
          if Calculation_Tuple[0] == 1:
               Prior_Prof_Loss = 0
               Calc_change_monthly = int(Prior_Prof_Loss + Calculation_Tuple[1])
               Calculations_For_Analysis.update({str("Prior_Profit_Loss"):Prior_Prof_Loss})
               Calculations_For_Analysis.update({str("Net_Profit_Loss_RollingTotal"):Calc_change_monthly})
          elif Calculation_Tuple[0] > 1:
               #I want this to do "prior profit + loss ... is from the row above ... "
...
#NOTE ... need to append another "looped dict" .... to my Loop_Placeholder ... from the TUPLE ... via => Loop_Placeholder.append(NEW_DICT - GENERATED by the NEW Looper)


#eval#for item in Loop_PlaceHolder: #NOTE I may not be able to use the Calculation_Tuple here yet ... have to look at the scenario for ... a "set" ... or "tuple" ... in case the syntax is different.
#eval#     for k, v in item.items():
#eval#          if k == str(header1a[1]):
#eval#               Prof_Loss_Change = {}
#eval#               row_change = 0
#               row_diff_to_prior = #NEED to get the formula...
#               #ALEX used => 
#               Calc_Row_Diff_ProfLoss = row_change + row_diff_to_prior  # "SOME VALUE CALCULATION"
#               Prof_Loss_Change.update({str("Profit_Loss_Change"):Calc_Row_Diff_ProfLoss})
#               if Calc_Row_Diff_ProfLoss == max(Calc_Row_Diff_ProfLoss): #NOTE> Might be best to do this "after the loop has appended to the parent "list" ... b/c in the future it will be a dataframe...
#                    Greatest_Increase 
#good#               Loop_PlaceHolder.append(Prof_Loss_Change)
#               print(v)
#ABOVE working to get just the column values ... via "munk" comment => https://stackoverflow.com/questions/22394783/how-to-access-dictionary-elements-in-a-list-in-python
...
#Analysis_DF = pd.DataFrame(Records_forAnalysis)
#Analysis_DF
#
#
#COMMENTING OUT ALL OF BELOW ... it's my "original code" ... but I am going to ATTEMPT to use ... the EMPTY LIST ... (csv_reader) ... populates DICT ... to List.APPEND ... b/c ... eventually I will be creating a "data frame" ... (or using the manual "header INDEX[0]/[1] values to "control" ... how I can produce these outputs/outcomes for my "LoB" ) ... "
#
#
