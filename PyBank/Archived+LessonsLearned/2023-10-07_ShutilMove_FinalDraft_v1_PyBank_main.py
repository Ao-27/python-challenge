...
#
#NEW EDITS .... OF PYSCRIPT FLOW ... PRIOR TO CREATION BELOW ... 
#+New BP file list URL => (in comments) ... https://9to5answer.com/move-and-replace-if-same-file-name-already-exists
#+New BP for exists via pathlib URL => https://stackoverflow.com/questions/45134102/shutil-move-if-directory-already-exists
#+oldBP - Move + Rename URL => https://pynative.com/python-move-files/
#+New KnowHow URL - Full Path Necessary => https://stackoverflow.com/questions/31813504/move-and-replace-if-same-file-name-already-exists
     #ALSO ... a comment on possibly using os.chdir() .... to TELL IT TO GO TO .... 
#+New Tutorial URL => https://note.nkmk.me/en/python-shutil-move/
#
     ##TEST LOGIC as Pseudo Code FIRST##
     #
     #1> Before writing the file ... should I ... i> Check for the output DIR to exist ... ii> Check if the PyScript has ran already and there's an existing file with the same name .... (Y+Y for both ...) ..
     #2> Check for existing file ... to move and rename => https://pynative.com/python-move-files/
#
     ##CONFIRMED LOGIC##
     #
     #0> CREATE PATHS for SOURCE AND DEST ... Dirs....
          #0a> FETCH the following:
               #0a1> dir_name
               #0a2> outputfilepath_1_name .... +2+3+4 ... 
                    #0a2a> MOVE THEM INTO .... FILE PATH list ... Data_FilesToFetch = [] ... (FOR LOOP) via => 
     #1> Check for if output DIR ("Analysis") exists ... 
          #1a> and if not... CREATE IT ... 
     #2> Check for if the Output filename1 (FILE BEING WRITTEN...) .... existing WITHIN the file .... 
          #2a> and (either remove it ... or rename to "archive ... via oldURL" => https://pynative.com/python-move-files/ )
          #2A!!> WE WILL MOVE AND RENAME if found ... 
#
#
...
#
###print(f"Before Moving File - open + write created file in: {script_dir_name}")  #NOTE: this is "path" ... in G for Geeks URL.
#
...
archive_location_postmove = os.path.join(archive_location_str) ### Needs the wrap in "str()" approach...   ### <<<= FixNote #SOMETHING WRONG ... to investigate ... it's going "c:" then my folder ..
#
#DOESN'T WORK ... #
...
#
if os.path.exists(target_location_premove + output1_file_name) == True and os.path.exists(original_location_premove) == True:
     try:
          try_source = target_location_premove + output1_file_name
          try:
               ...
               os.remove(target_location_premove+output1_file_name)     #"/"+output1_file_name)
...
#+NMewBP URL => https://stackoverflow.com/questions/31813504/move-and-replace-if-same-file-name-already-exists + https://pynative.com/python-move-files/
# Above is "looping" for file list appendage ... to finish each append by the "move" function ... I'm not aiming for that outcome at this time ... 
# #
#    src_dir = script_dir_name
#    move_file1_target = output1_file_name 
#    files_to_move = [move_file1_target] # EXPAND list as ... per VARs above ...
#    dst_dur_target = output_folder_dir
#
# PLUS addition of keeping "simplcity" ... from oldBP URL => 
# original_location = os.path.join(script_dir_name,output1_file_name)
# target_location = os.path.join(output_folder_dir)
#
# PLUS addition of For Loop via URL => https://pynative.com/python-move-files/
# # iterate files
...
#    # move file
#    shutil.move(source, destination)   #CHANGING to .... original_location, target_location .... 
#    print('Moved:', file)
#
#
...
#original_location =   script_dir_name+'/'+output1_file_name 
#NOTE this is the "sourcePath" for G for Geeks. Where "source" is the file for us ... and "path" ... is the location ...
#target_location = output_folder_dir      #script_dir_name+'/'+output_file_path1
