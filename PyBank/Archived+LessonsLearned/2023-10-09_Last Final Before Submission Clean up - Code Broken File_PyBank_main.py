...
#CREATE VARs to Create "Auto_Archive" ... for eventual Shutil "_new" on DATE1 ... etc. etc. use case per move ... 
auto_archive_folder_name = "Auto_Archived"
archive_location_str = str(output_folder_dir + "\\" + auto_archive_folder_name) # + "\\") # Note the "output_folder_dir" ... contains the "fetch py script" location ... at the start of it's OWN VAR ... making this more or less "dynamic" ... 

print(f"Auto_archive - OS.Path_Test1 prior to Shutil code block:\n{os.path.join(archive_location_str)}\n")
print(f"Prior to Folder Creations - Listing out the directories in current {output_folder_dir} -")
directories_in_analysis_output = list(filter(os.path.isdir,os.path.join(str(output_folder_dir))))      #os.walk(os.path.join(str(output_folder_dir)))))      # os.walk(archive_location_str)))
print(directories_in_analysis_output) 
print("-+-+-+-+-+-\n")
#print(os.walk(directories_in_analysis_output)) ##via URL => https://stackoverflow.com/questions/10989005/do-i-understand-os-walk-right
#ABOVE CODE ... really needs to be CLEANED UP ... but seems to be able to work for the "use case" .... I'm truly after but it's not "pythonic" ... (implicity vs explicity) ... etc. etc.

for output_dir in directories_in_analysis_output:   #Break+Continue Logic URL => https://www.programiz.com/python-programming/break-continue
     try:
          if (directories_in_analysis_output.index(auto_archive_folder_name)+1) >= 1: #FOUND somehwere in the list ... because first position is "0" ... it will give us "1" ... 
               print(f"The Output Directory folder Titled {auto_archive_folder_name} already exists.")
               continue#break #continue#break via URL => https://www.programiz.com/python-programming/break-continue
     except ValueError:
          print("Value Error - Index Cannot be found - Folder DIR NAME not in Paths' output VAR list")
     try:
          if [i for i in directories_in_analysis_output if output_dir == auto_archive_folder_name].count() >= 0:
               #os.mkdir(auto_archive_folder_name)
               print(f"The Output Directory folder Titled {auto_archive_folder_name} was created in\n{output_folder_dir}.")
     except TypeError:
          print("TypeError Bypassed - Purposefully - Code Cannot See 'non-created dir' at this time")
     except PermissionError:
          print(f"Bypass step creating Output Directory folder Titled:{auto_archive_folder_name}\nIt was created in:\n{output_folder_dir}.")
          os.remove(archive_location_str)
          os.mkdir(archive_location_str)
     print(f"\n{list(filter(os.path.isdir,os.path.join(str(archive_location_str))))}")
print("-+-Auto_Archived_For Loop End-+-\n")
     ...
     #directories_in_curdir.index(output_folder_name) <= 0: #NOT FOUND anywhere in the list ... because index() function produces "-1" as it's "N/A" ...  #wrong ...it produces a "ValueError" see URL => https://docs.python.org/3/tutorial/datastructures.html
          #CREATE FOLDER.... since .... "no dir with the output_folder_name  ... was found producing "0" ... 
                    ...
                    ##PERS MOD ... using os.walk() ...  # os.listdir(os.curdir))) 
               #via URL => https://www.w3schools.com/python/ref_func_filter.asp
               #Above is using the "Filter" function ... the "isdir" ... is checking to see if ... the current directory ... is within the list of "all found directories in the current one ... "
               #NOTE ... using my VARs produces a "str" "TypeError" ...  #list(filter(script_dir_name, os.listdir(os.curdir))) #
...
original_location_premove = os.path.join(script_dir_name,output1_file_name) #Cannot "be ideal" ... and wrap this in Path() here ... b/c of my f-strings using it below ... I'll have to "hard code" ... it by writing the wrapping of Path() ...  into the shutil.move() block ...
...
if os.path.exists(Path(target_location_premove + '/' + output1_file_name)):  #via URL => https://stackoverflow.com/questions/45134102/shutil-move-if-directory-already-exists
     #OLD CODE => #(target_location_premove + output1_file_name):# == True and/& os.path.exists(original_location_premove) == True:
          ...
          try: 
               shutil.copy2(try_source,Path(archive_location_postmove)) #via URL => https://www.geeksforgeeks.org/python-shutil-copy2-method/
     ...
     #    shutil.move(src_folder + file_name, new_name)
               src_filename = Path(os.path.join(script_dir_name, os.path.basename(output1_file_name)))
               dst_filename = Path(os.path.join(output_folder_dir, os.path.basename(output1_file_name)))
               shutil.move(
                           os.path.join(src_filename) #script_dir_name, output1_file_name)
                           ,
                           os.path.join(dst_filename) #output_folder_dir, output1_file_name)
                           )
          #Actually ... I need to use via URL => https://stackoverflow.com/questions/31813504/move-and-replace-if-same-file-name-already-exists
     #ABOVE becomes "redundant" ... the copy2 ... has already "copied" ... the newly created file in {script_dir_name} ... into Target ... 
     #ACTUALLY ... that cannot be "true" b/c ... copy2() DOES NOT ... even point to ... {script_dir_name} ...
...
if not os.path.exists(target_location_premove + output1_file_name):       
     # != True and/& os.path.exists(original_location_premove) == True:
     print(f"Source File Folder Contents - Before File Move:\n{os.listdir(script_dir_name)}") #via URL => https://www.geeksforgeeks.org/python-move-and-overwrite-files-and-folders/    
     try:
