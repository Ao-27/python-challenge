...
for candidate in Candidates_List:
     ...
     Candidate_Votes_Max = {} #{"Candidates_Votes_Looped":{{Dict_key1:[]},{Dict_key2:[]},{Dict_key3:[]}}}}
...
#below via URL => https://stackoverflow.com/questions/6987285/find-the-item-with-maximum-occurrences-in-a-list

#Winner =  max(Candidates_List,key=Candidates_List.count) #max(len(Candidates_List) ,key=Candidates_List.count)


#Below via URL => https://datagy.io/python-get-dictionary-key-with-max-value/

#Winner = max([max(dict.values()) for d in Votes_Placeholder])

#Below via URL => https://stackoverflow.com/questions/31751651/max-value-in-dictionary-of-list-of-dictionaries-in-python
#
#WRONG BELOW ... but play around with it to produce different results...
#Winner = max(Candidate_Votes_Max.items())

#Below via URL => https://stackoverflow.com/questions/36516279/how-to-return-elements-with-the-highest-occurrence-in-list

vals = max(Candidate_Votes_Max.values())
Winner = [k for k,v in Candidate_Votes_Max.items() if v == vals]
print(f"The Winner of the Election is:\n{Winner}")
print(f"manual debug - {Candidate_Votes_Max.values()}\nAbove are the values being considered.")
print(f"manual debug - {Candidate_Votes_Max.items()}\nAbove are the items being considered.")
#ABOVE = same outcome as before ... I must have LOOPED in correctly ... to append/update the dict ?? 
#
#


#
#See URLs for below => https://stackoverflow.com/questions/17657720/python-list-comprehension-double-for?noredirect=1&lq=1
#https://stackoverflow.com/questions/1198777/double-iteration-in-list-comprehension?noredirect=1&lq=1
#
#Winner = [x for x in Candidates_List for winner in ]
#print(f"")
#
